...
[快速体验](#快速体验) · [产品闭环](#这个项目解决什么问题) · [AI-Token-实验](#ai-token-实验) · [项目结构](#项目结构) · [文档与实现边界](#文档与实现边界)
...
本项目使用 MeterForge 名称。功能与运行行为应以本仓库源码、测试和本地部署结果为准；外部产品的说明、云服务、SDK 发布状态不应被视为本项目已经实现或提供的能力。

外部网站的离线文档快照不纳入项目文档树。第三方依赖的真实模块路径以及历史数据库迁移中的兼容标识，不进行仅为统一品牌的替换。具体范围见 [项目命名与文档边界](docs/project-scope.md)。
