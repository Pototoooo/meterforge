...
You are helping the user modify the MeterForge database schema and generate a corresponding migration.

## Context

- **Schema files:** `meterforge/ent/schema/*.go` — ent schema definitions (source of truth)
- **Generated ent code:** `meterforge/ent/db/` — DO NOT edit manually
...
Edit or create files in `meterforge/ent/schema/`. Look at existing schema files for conventions.
...
This runs `go generate ./...` which regenerates the ent client code in `meterforge/ent/db/` from the schema definitions. Check that it completes without errors.
...
For fields, edges (relationships), and indexes, **read existing schemas** in `meterforge/ent/schema/` for conventions. Key things to know:
...
- Never edit files in `meterforge/ent/db/` manually
