...
## MeterForge-local naming conventions (not from AIP-122)
...
| ---------------------------- | ------------ | --------------------------------------- | --------------------------------------------- |
| URL paths                    | `kebab-case` | `/api/v3/meterforge/llm-cost`            | AIP-122                                       |
...
| Model names (TypeSpec)       | `PascalCase` | `BillingProfile`                        | MeterForge linter only                         |
| Enum type names (TypeSpec)   | `PascalCase` | `MeterAggregation`                      | MeterForge linter only                         |
| Enum member names (TypeSpec) | `PascalCase` | `UniqueCount`                           | MeterForge linter only                         |
| Path parameters (TypeSpec)   | `camelCase`  | `meterId`, `customerId`                 | MeterForge linter only                         |

The TypeSpec-facing casing (`PascalCase` model names, `camelCase` path parameters) is an MeterForge convention because TypeSpec model identifiers and path parameter identifiers are separate from the JSON wire format. The wire format still complies with AIP-122.

## Base resource models (not from AIP-122)

These are MeterForge `Shared.Resource` conventions defined in `api/spec/packages/aip/src/shared/resource.tsp`, not AIP-122 rules. They use AIP-122-compliant snake_case field names on the wire.
