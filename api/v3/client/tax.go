...
func (s *TaxService) GetCode(ctx context.Context, taxCodeID string) (*TaxCode, error) {
	if taxCodeID == "" {
		return nil, fmt.Errorf("meterforge: %s must not be empty: %w", "taxCodeID", ErrEmptyID)
	}

	path := "/meterforge/tax-codes/{taxCodeId}"
...
func (s *TaxService) DeleteCode(ctx context.Context, taxCodeID string) error {
	if taxCodeID == "" {
		return fmt.Errorf("meterforge: %s must not be empty: %w", "taxCodeID", ErrEmptyID)
	}
