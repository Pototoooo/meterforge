...
func TestAPIErrorParsesRFC7807(t *testing.T) {
	...
	body := `{"status":404,"type":"https://github.com/Pototoooo/meterforge/problems/not-found","title":"Not Found","detail":"meter not found","instance":"kong:trace:abc123"}`
	rec := &requestRecorder{}
	mf := newTestClient(t, rec.handler(http.StatusNotFound, body))
	...
	if apiErr.Type != "https://github.com/Pototoooo/meterforge/problems/not-found" {
	...
	if want := "meterforge: 404 Not Found: meter not found"; apiErr.Error() != want {
...
func TestAPIErrorTitleOnly(t *testing.T) {
	...
	mf := newTestClient(t, (&requestRecorder{}).handler(http.StatusTeapot, `{"status":418,"title":"Teapot"}`))
	...
	if want := "meterforge: 418 Teapot"; apiErr.Error() != want {
		t.Errorf("Error() = %q, want %q", apiErr.Error(), want)
	}
}
...
func TestAPIErrorNonJSONBodyFallsBackToRawEcho(t *testing.T) {
	...
	t.Run("short body echoed in full", func(t *testing.T) {
		...
		mf := newTestClient(t, (&requestRecorder{}).handler(http.StatusBadGateway, body))
		...
		if want := "meterforge: unexpected status 502: " + body; apiErr.Error() != want {
			t.Errorf("Error() = %q, want %q", apiErr.Error(), want)
		}
	})
	...
	t.Run("long body truncated at 512 bytes", func(t *testing.T) {
		...
		want := "meterforge: unexpected status 502: " + strings.Repeat("x", 512) + "… (truncated)"
...
func TestAsAPIError(t *testing.T) {
	...
	mf := newTestClient(t, (&requestRecorder{}).handler(http.StatusInternalServerError, `{"status":500,"title":"Boom"}`))
	...
	apiErr, ok := meterforge.AsAPIError(wrapped)
	if !ok {
	...
	if got, ok := meterforge.AsAPIError(errors.New("plain")); ok || got != nil {
...
func TestDecodeAPIError(t *testing.T) {
	...
	t.Run("decodes a typed error body", func(t *testing.T) {
		...
		mf := newTestClient(t, (&requestRecorder{}).handler(http.StatusBadRequest, body))

		_, err := mf.Meters.Get(t.Context(), "m-1")
		problem, ok, decodeErr := meterforge.DecodeAPIError[validationProblem](err)
		if decodeErr != nil {
	...
	t.Run("non-API errors are reported as not decodable", func(t *testing.T) {
		problem, ok, decodeErr := meterforge.DecodeAPIError[validationProblem](errors.New("dial tcp: refused"))
		if ok || decodeErr != nil {
	...
	t.Run("undecodable body surfaces the decode error", func(t *testing.T) {
		mf := newTestClient(t, (&requestRecorder{}).handler(http.StatusInternalServerError, "not json"))

		_, err := mf.Meters.Get(t.Context(), "m-1")
		_, ok, decodeErr := meterforge.DecodeAPIError[validationProblem](err)
		if !ok {
			t.Fatal("DecodeAPIError reported the error is not an APIError")
		}
		if decodeErr == nil {
...
func TestEmptyIDGuard(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			wantParam: "meterID",
			call: func(ctx context.Context) error {
				_, err := mf.Meters.Update(ctx, "", meterforge.UpdateMeterRequest{})
				return err
			},
		},
		{
			name:      "meters delete",
			wantParam: "meterID",
			call:      func(ctx context.Context) error { return mf.Meters.Delete(ctx, "") },
		},
		{
			name:      "customers get",
			wantParam: "customerID",
			call: func(ctx context.Context) error {
				_, err := mf.Customers.Get(ctx, "")
				return err
			},
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !errors.Is(err, meterforge.ErrEmptyID) {
