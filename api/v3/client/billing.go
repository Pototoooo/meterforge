...
func (s *BillingService) GetProfile(ctx context.Context, id string) (*Profile, error) {
	if id == "" {
		return nil, fmt.Errorf("meterforge: %s must not be empty: %w", "id", ErrEmptyID)
	}

	path := "/meterforge/profiles/{id}"
...
func (s *BillingService) DeleteProfile(ctx context.Context, id string) error {
	if id == "" {
		return fmt.Errorf("meterforge: %s must not be empty: %w", "id", ErrEmptyID)
	}
