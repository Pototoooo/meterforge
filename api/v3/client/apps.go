...
func (s *AppsService) Get(ctx context.Context, appID string) (*App, error) {
	if appID == "" {
		return nil, fmt.Errorf("meterforge: %s must not be empty: %w", "appID", ErrEmptyID)
	}

	path := "/meterforge/apps/{appId}"
...
func (s *AppsService) Uninstall(ctx context.Context, appID string) error {
	if appID == "" {
		return fmt.Errorf("meterforge: %s must not be empty: %w", "appID", ErrEmptyID)
	}
...
func (s *AppsService) GetCatalogItem(ctx context.Context, appType string) (*AppCatalogItem, error) {
	if appType == "" {
		return nil, fmt.Errorf("meterforge: %s must not be empty: %w", "appType", ErrEmptyID)
	}

	path := "/meterforge/app-catalog/{appType}"
...
func (s *AppsService) Install(ctx context.Context, request InstallAppRequest) (*InstallAppResponse, error) {
	path := "/meterforge/app-catalog/install"
