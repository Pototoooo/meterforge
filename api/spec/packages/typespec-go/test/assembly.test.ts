...
describe('runtime template assembly', () => {
  ...
  it('interpolates the module path into resolveVersion and the sdk-version fallback into its return', () => {
    ...
    expect(option).toContain(
      'dep.Path != "github.com/Pototoooo/meterforge/api/v3/client"',
    )
...
describe('generated README', () => {
  ...
  it('shows compiling examples with an mf variable and meterforge qualifiers', async () => {
    const { readme } = await compileReadme()

    expect(readme).toContain('mf, err := meterforge.New(')
    expect(readme).toContain(
      'mf.Meters.Create(ctx, meterforge.CreateMeterRequest{',
    )
    expect(readme).toContain('meterforge.MeterListParams{}')
  ...
  it('lists the Stream variant of text-response operations', async () => {
    ...
    for (const methodName of methodNames) {
      expect(readme).toContain(`\`mf.Test.${methodName}\``)
    }
    expect(readme).toContain(
      `\`mf.Test.${goExportedName(csvMethodName)}Stream\``,
    )
  })
  ...
  it('documents opaque cursor tokens and backward-iteration ordering', async () => {
    ...
    expect(readme).toContain(
      'mf.Meters.QueryCSVStream(ctx, "meter-id", meterforge.MeterQueryRequest{})',
    )
  })
})
