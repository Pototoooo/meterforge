...
/**
 * Emits the MeterForge Go SDK from the AIP TypeSpec program.
 *
...
export async function $onEmit(context: EmitContext<GoEmitterOptions>) {
  ...
  if (!modulePath) {
    throw new Error(
      "typespec-go: the module-path option is required; set it to the SDK's published Go module path (e.g. github.com/Pototoooo/meterforge/api/v3/client) in tspconfig.yaml",
    )
  }
  if (!pkg) {
    throw new Error(
      'typespec-go: the package-name option is required; set it to the generated Go package name (e.g. meterforge) in tspconfig.yaml',
    )
  }
...
export function prepareRuntimeTemplate(
  ...
  return `${generatedHeader}\n\n${content
    .replace(/^package meterforge/m, `package ${pkg}`)
    .replaceAll('{{MODULE_PATH}}', modulePath)
...
export function packageDocHeader(pkg: string): string {
  return (
    [
      generatedHeader,
      '',
      `// Package ${pkg} provides a Go client SDK for the MeterForge API — usage`,
      '// metering and billing for AI and DevTool companies. It is generated from the',
      '// MeterForge TypeSpec definitions and exposes every operation as a method on a',
      ...
      `//		${pkg}.WithToken(os.Getenv("METERFORGE_API_KEY")),`,
      '//	)',
      '//',
