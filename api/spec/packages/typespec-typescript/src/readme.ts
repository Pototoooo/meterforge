...
function header(note?: string): string {
  const lines = [
    '# MeterForge SDK',
    '',
    'TypeScript client for the MeterForge API — usage metering and billing for',
...
function initialization(packageName: string): string {
  return [
    ...
    '',
    '```typescript',
    `import { MeterForge } from '${packageName}'`,
    '',
    'const client = new MeterForge({',
    "  baseUrl: 'http://localhost:8888/api/v3',",
    '  apiKey: process.env.METERFORGE_API_KEY,',
    '})',
    '```',
    '',
    ...
    '',
    '```typescript',
    `import { MeterForge, ServerList } from '${packageName}'`,
    '',
    'const client = new MeterForge({',
...
function runtimeValidation(packageName: string): string {
  return [
    ...
    '',
    '```typescript',
    `import { MeterForge, ValidationError } from '${packageName}'`,
    '',
    'const client = new MeterForge({',
    "  baseUrl: 'http://localhost:8888/api/v3',",
    '  apiKey: process.env.METERFORGE_API_KEY,',
...
function errorHandling(packageName: string): string {
  return [
    ...
    '',
    '```typescript',
    `import { MeterForge, HTTPError } from '${packageName}'`,
    '',
    'const client = new MeterForge({',
    "  baseUrl: 'http://localhost:8888/api/v3',",
    '  apiKey: process.env.METERFORGE_API_KEY,',
