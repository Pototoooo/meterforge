...
func TestListSubjects(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantSQL:  "SELECT DISTINCT subject FROM meterforge.mf_events WHERE namespace = ? ORDER BY subject",
		...
		},
		{
			...
			wantSQL:  "SELECT DISTINCT subject FROM meterforge.mf_events WHERE namespace = ? AND type = ? ORDER BY subject",
		...
		},
		{
			...
			wantSQL:  "SELECT DISTINCT subject FROM meterforge.mf_events WHERE namespace = ? AND type = ? AND time >= ? ORDER BY subject",
		...
		},
		{
			...
			wantSQL:  "SELECT DISTINCT subject FROM meterforge.mf_events WHERE namespace = ? AND type = ? AND time >= ? AND time < ? ORDER BY subject",
		...
		},
		{
			...
			wantSQL:  "SELECT DISTINCT subject FROM meterforge.mf_events WHERE namespace = ? AND positionCaseInsensitive(subject, ?) > 0 ORDER BY subject",
