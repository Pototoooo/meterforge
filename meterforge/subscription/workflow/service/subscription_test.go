...
import (
	...
	"github.com/Pototoooo/meterforge/meterforge/subscription/patch"
	...
	workflowservice "github.com/Pototoooo/meterforge/meterforge/subscription/workflow/service"
...
func TestSettlementMode(t *testing.T) {
	...
	t.Run("CreateFromPlan with CreditOnly plan is rejected when credits are disabled in feature gate", func(t *testing.T) {
		...
		ctx := context.WithValue(context.Background(), featuregate.FeatureFlag("mf_ff_credits_enabled"), false)
		...
		require.ErrorContains(t, err, "validation error: credits are not enabled on this deployment of MeterForge")
	})
	...
	t.Run("CreateFromPlan with CreditOnly plan succeeds when credits are enabled in feature gate", func(t *testing.T) {
		...
		ctx := context.WithValue(context.Background(), featuregate.FeatureFlag("mf_ff_credits_enabled"), true)
