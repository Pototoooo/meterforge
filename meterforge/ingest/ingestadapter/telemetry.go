...
func (c *collectorTelemetry) Ingest(ctx context.Context, namespace string, ev event.Event) error {
	...
	ctx, span := c.tracer.Start(ctx, "meterforge.ingest.events", trace.WithAttributes(
		namespaceAttr,
		attribute.String("meterforge.event.id", ev.ID()),
	))
	defer span.End()
