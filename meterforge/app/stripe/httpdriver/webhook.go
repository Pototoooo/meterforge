	...
	return httptransport.NewHandlerWithArgs(
		...
		func(ctx context.Context, request AppStripeWebhookRequest) (AppStripeWebhookResponse, error) {
			...
			case stripeclient.WebhookEventTypeSetupIntentSucceeded:
				...
				// If the event has not app metadata it's not initiated by an MeterForge app and we ignore it.
				// This can be the case when someone manually creates a payment intent.
				if !hasMetadataAppId {
					return AppStripeWebhookResponse{
						Message: lo.ToPtr("ignoring event as it was not initiated by the meterforge app"),
					}, nil
				}

				// When the MeterForge app id is set it cannot be empty
				if metadataAppId == "" {
					return AppStripeWebhookResponse{}, models.NewGenericValidationError(
				...
				if metadataAppId != request.AppID.ID {
					...
					return AppStripeWebhookResponse{
						Message: lo.ToPtr("ignoring event as it was initiated by a different meterforge app"),
					}, nil
				}
