...
// Service manages exporting data from MeterForge meters
type Service interface {
	...
	// ExportSyntheticMeterData exports synthetic pre-aggregated events from MeterForge. When ingested into a meter matching the descriptor, the resulted events will accurately reconstruct the meter histogram with WindowSize precision.
	// ExportSyntheticMeterData produces one event per WindowSize using the same event format as the storage layer.
	// This pre-aggregation is useful because while MeterForge is designed to handle large event volumes, downstream systems usually don't care about the full granularity of all stored events.
