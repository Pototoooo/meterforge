...
MeterForge ingests usage data through events using the [CloudEvents](https://cloudevents.io/) specification incubated under CNCF.
...
As we mentioned in the intro, MeterForge ingests events using the CloudEvents specification from CNCF.
...
MeterForge currently supports the [JSON format](https://github.com/cloudevents/spec/blob/main/cloudevents/formats/json-format.md) of CloudEvents, with plans to extend support for Protobuf and other formats.
...
SDKs are available for common programming languages, simplifying the creation, validation, and reporting of usage events to MeterForge.

## Event Processing

MeterForge continuously processes usage events, allowing you to update meters in real-time. Once an event is ingested, MeterForge aggregates the data based on your defined meters.
For example, you can define a meter called "Parallel jobs", and MeterForge will aggregate the maximum number of jobs by each customer over a given time period.

Using an example, let’s dive into how MeterForge’s event processing works.
...
The meter config above tells MeterForge to expect CloudEvents with `type=api-calls` where the usage value is stored in `data.duration` and we need to sum them by `data.path`.
MeterForge will track the usage value for every time window when at least one event was reported and track it for every `subject` and `groupBy` permutation.
...
MeterForge will track the usage value for the time window and customer as:
...
MeterForge will increase sum of the duration for the two events for the same time window (`windowstart`, `windowend`), `$.path` and `subject`:
...
MeterForge deduplicates events by id and source, by default, for 32 days. This ensures that if multiple events with the same id and source are sent, they will be processed only once.
