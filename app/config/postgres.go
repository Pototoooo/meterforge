...
func (a AutoMigrate) Validate() error {
	...
	case AutoMigrateMigration, AutoMigrateMigrationJob, AutoMigrateOff:
		return nil
	case "ent":
		return errors.New("ent auto-migration is no longer supported; run 'meterforge-jobs migrate adopt-ent' once, then run the normal migration (see docs/database-migration.md)")
	default:
